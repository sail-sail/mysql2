...
import { createHash, publicEncrypt } from "../../deps.ts";
import { xor, xorRotating } from "../auth_41.ts";
...
function sha256(msg: string | Buffer) {
  const hash = createHash('sha256');
  hash.update(msg, "binary");
  return hash.digest("binary") as Buffer;
}
        ...
        if (PERFORM_FULL_AUTHENTICATION_PACKET.equals(data)) {
          const isSecureConnection =
            typeof pluginOptions.overrideIsSecure === 'undefined'
              ? (connection.config as any).ssl || connection.config.socketPath

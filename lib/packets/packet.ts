...
import ErrorCodeToName from "../constants/errors.ts";
...
function leftPad(num: number, value: string|number) {
...
class Packet {
  ...
  readInt64() {
    ...
    const res2 = resNumber.toString() === resString ? resNumber : resString;
    return res2;
  }
  ...
  readLengthCodedNumberExt(tag: number, bigNumberStrings?: boolean, signed?: boolean) {
    let word0, word1;
    if (tag === 0xfb) {
      return null;
    }
    ...
    if (tag === 0xfe) {
      ...
      const res = new Long(word0, word1, !signed); // Long need unsigned
      const resNumber = res.toNumber();
      const resString = res.toString();
      const res2 = resNumber.toString() === resString ? resNumber : resString;
      return bigNumberStrings ? resString : res2;
    }
    // eslint-disable-next-line no-console
    console.trace();
  ...
  readLengthCodedBuffer() {
    ...
    return this.readBuffer(len as number);
  }
  ...
  parseGeometryValue() {
    const buffer0 = this.readLengthCodedBuffer();
    let offset = 4;
    if (buffer0 === null || !buffer0.length) {
      return null;
    }
    const buffer = buffer0 as Buffer;
    // deno-lint-ignore no-explicit-any
    function parseGeometry(): any {
  ...
  parseDate(timezone?: string) {
    ...
    const y = this.parseInt(4) as number;
    this.offset++; // -
    const m = this.parseInt(2) as number;
    this.offset++; // -
    const d = this.parseInt(2) as number;
  ...
  parseFloat(len?: number) {
    ...
    while (this.offset < end) {
      ...
      if (charCode === dot) {
        ...
        this.offset++;
        const exponentValue = this.parseInt(end - this.offset) as number;
  ...
  parseLengthCodedIntNoBigCheck() {
    return this.parseIntNoBigCheck(this.readLengthCodedNumber() as number);
  }

  parseLengthCodedInt(supportBigNumbers?: boolean) {
    return this.parseInt(this.readLengthCodedNumber() as number, supportBigNumbers);
  }
  ...
  parseLengthCodedFloat() {
    return this.parseFloat(this.readLengthCodedNumber() as number);
  }
  ...
  asError(encoding?: string) {
    ...
    // deno-lint-ignore no-explicit-any
    (err as any).code = ErrorCodeToName[errorCode];
    // deno-lint-ignore no-explicit-any
    (err as any).errno = errorCode;
    // deno-lint-ignore no-explicit-any
    (err as any).sqlState = sqlState;
    // deno-lint-ignore no-explicit-any
    (err as any).sqlMessage = message;
    return err;
  }
